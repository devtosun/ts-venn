...
export function Canvas() {
  ...
  const renderOrder = useAppState((s) => s.renderOrder);
  const regions = useAppState((s) => s.regions);
  const segmentList = useAppState((s) => s.segmentList);
  const dragging = useAppState((s) => s.dragging);
  const resizing = useAppState((s) => s.resizing);

  // Select actions
  const startDrag = useAppState((s) => s.startDrag);
  const drag = useAppState((s) => s.drag);
  const endDrag = useAppState((s) => s.endDrag);
  const startResize = useAppState((s) => s.startResize);
  const resize = useAppState((s) => s.resize);
  const endResize = useAppState((s) => s.endResize);
  const getSegmentAt = useAppState((s) => s.getSegmentAt);
  const getRegionAt = useAppState((s) => s.getRegionAt);
  const selectSegment = useAppState((s) => s.selectSegment);
  const toggleRegion = useAppState((s) => s.toggleRegion);
